...
setHeader({ title: 'Open Projects', icon: 'i-lucide-folder-open' })
...
// ─── Excluded statuses (case-insensitive) ────────────────────
const EXCLUDED_STATUSES = ['cancelled', 'closed', 't/s']
...
const baseProjects = computed(() => {
  return projects.value.filter((p: any) => {
    ...
    if (EXCLUDED_STATUSES.includes(jobStatus)) return false
