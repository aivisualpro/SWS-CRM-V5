...
// Helper: split comma-separated Project Status into individual trimmed values
...
const projectStatusTabs = computed(() => {
  ...
  for (const p of projects.value) {
    const statuses = getProjectStatuses(p)
    for (const s of statuses) {
      counts[s] = (counts[s] || 0) + 1
    }
  }
  // Sort alphabetically, put Unknown at end
  ...
  return [
    { label: 'All', count: projects.value.length },
...
const filteredProjects = computed(() => {
  ...
  // Tab filter — match if any of the project's comma-separated statuses equals the active tab
  if (activeTab.value !== 'All') {
    list = list.filter((p: any) => {
      const statuses = getProjectStatuses(p)
      return statuses.includes(activeTab.value)
    })
  }
